...
Housekeeping debt: seven copies of the markdown `cell`/`table`/`code` helpers remain (FU.1). The orphaned per-package
...
- [x] **3.1 Tokens fragment for the intro prose (done 2026-07-13).** `packages/tokens/projects/tokens-meta/skill/` now
      follows the styles pattern (`intro.md` + `how-to-use.md` + serializer over `tokensMeta`), exporting
      `tokensSkill: PackageSkill` (`name: 'tokens'`; `intro` owns the `## Tokens` SKILL.md section prose, the pointer
      sentence stays aggregator-owned) with its own snapshot spec. The storefront `tokensReference.ts` dissolved into
      the `packageSkills.ts` registrations (mount root → `references/tokens.md`); trees regenerated byte-identical. The
      storefront tokens introduction page is untouched (dedup: FU.5).
...
- [ ] **FU.1 Hoist the markdown helpers to `packages/shared`.** Seven copies of `cell`/`table`/`code`:
      `storefront/src/lib/skill/support/markdown.ts` + the six fragment `skill/skill.ts` files (the tokens fragment
      added a copy of the storefront's stricter helpers in task 3.1). Unify on the storefront's stricter semantics
      (`escapeCell` collapses all whitespace + trims; the styles `cell` only collapses newlines). Fix: export from
      `packages/shared` (already a dependency of all three packages); import everywhere; delete local copies. Must
      resolve under the storefront `tsx` runtime (deep source imports) — mirror how `stylesReference.ts` imports
      fragment source today. Acceptance: one implementation; the compact tree hashes prove output-neutral (or show an
      intentional generated-content change).
