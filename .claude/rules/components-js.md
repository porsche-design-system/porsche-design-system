...
All cross-component tests live here in four suites, each with its own `config/playwright.config.ts`:
...
const initButton = (page: Page, opts: { type?: string } = {}): Promise<void> => {
  const { type = 'button' } = opts;
  const content = `<p-button type="${type}">Click me</p-button>`;
...
test.describe('button', () => {
  for (const scheme of schemes) {
    for (const viewportWidth of viewportWidths) {
      test(`should match screenshot for scheme=${scheme} viewport=${viewportWidth}`, async ({ page }) => {
...
- `setupScenario(page, url, viewportWidth)` — navigate + set viewport
- Auto-focus revert for overlay components (modals, flyouts)
- Tests sharded 6 ways in CI for parallel execution
...
test.describe('button', () => {
  for (const scheme of schemes) {
    for (const viewport of [viewportWidthXXS, viewportWidthM]) {
      test(`should have no violations for scheme=${scheme} viewport=${viewport}`, async ({ page }) => {
...
A11y tree snapshot tests compare the accessibility tree structure against saved baselines.
...
- **CLI** (`projects/components-manager-cli/`): Build tool for component chunks
- **Core** (`projects/components-manager-core/`): Runtime that lazy-loads component definitions
- Uses custom Webpack `CustomNamedChunkIdsPlugin` for deterministic chunk names
- Output format: `porsche-design-system.v{version}.{contenthash}.js`
