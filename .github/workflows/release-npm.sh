...
publish_npmjs() {
  ...
  if [[ ${NPM_TAG_NAME} ]]; then
    result=$(npm run publish --tag ${NPM_TAG_NAME} --non-interactive --registry=https://registry.npmjs.org/ --access public "${PACKAGE_LOCATION}")
  else
    # tagged with "latest"
    result=$(npm run publish --non-interactive --registry=https://registry.npmjs.org/ --access public "${PACKAGE_LOCATION}")
  fi
