...
## Release management - Porsche Design System (JS/Angular/React)
...
1. Execute `yarn version --no-git-tag-version` and enter new patch or minor version
1. Update Design System JS dependency to corresponding version number (must always be pinned)
...
### Commit
1. Create a commit with following message structure `Release Porsche Design System (JS/Angular/React) v{MAJOR_NUMBER}.{MINOR_NUMBER}.{PATCH_NUMBER} | {DEVELOPER_ABBREVEATION}`
...
### Publish
1. Switch to __packages/ui-kit-js directory__
1. Run `yarn publish --registry=https://porscheui.jfrog.io/porscheui/api/npm/npm-local/` which will deploy the Design System JS artifact to the Artifactory repository.
1. Switch to __packages/ui-kit-angular/dist/ui-kit-wrapper directory__ (make sure to release package within **dist** folder)
1. Run `yarn publish --registry=https://porscheui.jfrog.io/porscheui/api/npm/npm-local/` which will deploy the Design System Angular artifact to the Artifactory repository.
...
1. Run `yarn publish --registry=https://porscheui.jfrog.io/porscheui/api/npm/npm-local/` which will deploy the Design System React artifact to the Artifactory repository.
...
## Release management - Porsche Design System (SCSS Utils)
...
## Release management - Porsche Design System (Sketch Libraries)
