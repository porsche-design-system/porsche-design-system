...
# Input Email (Component: p-input-email)
...
export const Example = () => {
  return (
    <>
      <PInputEmail label="Some label" name="some-name" indicator={true}></PInputEmail>
    </>
  )
}
```
...
The `p-input-email` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your
needs.
...
In the **uncontrolled** approach, the input behaves similar to a native `<input type="email" />`, automatically managing
