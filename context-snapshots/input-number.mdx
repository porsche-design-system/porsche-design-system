...
# Input Number (Component: p-input-number)
...
export const Example = () => {
  return (
    <>
      <PInputNumber label="Some label" name="some-name" controls={true}></PInputNumber>
    </>
  )
}
```
...
The `p-input-number` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your
needs.
...
In the **uncontrolled** approach, the input behaves similar to a native `<input type="number" />`, automatically
