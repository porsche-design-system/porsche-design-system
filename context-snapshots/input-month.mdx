...
# Input Month (Component: p-input-month)
...
export const Example = () => {
  return (
    <>
      <PInputMonth label="Some label" name="some-name"></PInputMonth>
    </>
  )
}
```
...
direct keyboard input, including arrow-key adjustments for precise changes. It has a start and end slot in order to
support flexible slotted content.

A `label` is a caption which informs the user what information a particular form field is asking for. The
`p-input-month` component can be used with or without a label, but it's recommended to keep the label visible for better
...
The `p-input-month` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your
needs.
...
In the **uncontrolled** approach, the input behaves similar to a native `<input type="month" />`, automatically managing
