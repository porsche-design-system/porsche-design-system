...
gulp.task("pl-copy:js", function() {
    return gulp.src("**/*.js", { cwd: resolvePath(paths().source.js) }).pipe(gulp.dest(resolvePath(paths().public.js)))
})
...
gulp.task("pl-lint:porsche-stylesheet", function() {
    return gulp
        .src(resolvePath(paths().source.porscheStylesheets) + "/src/**/*.s+(a|c)ss")
        .pipe(cache("linting"))
        .pipe(
            sassLint({
...
gulp.task(
    ...
    gulp.series("pl-build:porsche-stylesheets", function() {
        return gulp
            .src(resolvePath(paths().source.porscheStylesheets) + "/dist/**/*")
            .pipe(gulp.dest(resolvePath(paths().public.css)))
            .pipe(browserSync.stream())
    })
)
...
gulp.task(
    "patternlab:build",
    gulp.series("pl-assets", build, "pl-lint:porsche-stylesheet", function(done) {
        done()
    })
)
...
function watch() {
    gulp
        .watch(resolvePath(paths().source.css) + "/**/*.css", { awaitWriteFinish: true })
        .on("change", gulp.series("pl-copy:css", reloadCSS))
    gulp
        .watch(resolvePath(paths().source.porscheStylesheets) + "/src/**/*.scss", { awaitWriteFinish: true })
        .on("change", gulp.series("pl-copy:porsche-stylesheets", "pl-lint:porsche-stylesheet", reloadCSS))
    gulp
        .watch(resolvePath(paths().source.js) + "/**/*.js", { awaitWriteFinish: true })
        .on("change", gulp.series("pl-copy:js", reload))
    gulp
