...
const generateCssAndHtml = (overlayType: OverlayType): void => {
  ...
  const targetFile = `./src/${overlayType}-support/${overlayType}-support.ts`;
  ...
  const browserOverlayMarkup =
    "<p>${content.replace('<br>','<br class=show--at-768-ilb>')}</p><div class=show--at-768>${[chrome, firefox, edge].map((link) => link.replace('>', '><i></i>')).join('')}</div>";

  const cookieOverlayMarkup = '<p>${content}</p>';
  ...
  const htmlMarkup = \`<strong>\${title}</strong>${isBrowser ? browserOverlayMarkup : cookieOverlayMarkup}\`;
