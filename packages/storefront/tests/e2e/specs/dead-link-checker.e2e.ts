...
describe('check for dead links', () => {
  ...
  const getMainLinks = () => page.$$('main [href]');
  ...
  const scanForLinks = async (): Promise<string[]> => {
    ...
    const mainLinks = await getMainLinks();
    const mainHrefs = await Promise.all(mainLinks.map((x) => x.evaluate(getHref)));

    const mainHrefsStartingWithSlash = mainHrefs.filter((url) => url.startsWith('/'));
    expect(mainHrefsStartingWithSlash.length).toBe(0, 'mainHrefsStartingWithSlash.length');
    if (mainHrefsStartingWithSlash.length) {
      console.error('Link(s) starting with "/" were found:', mainHrefsStartingWithSlash);
    }
