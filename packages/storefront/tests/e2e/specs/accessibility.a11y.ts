...
describe('within main element', () => {
  ...
  const cycleFrameworkTabs = async (theme: string): Promise<void> => {
    ...
    if (buttons.length) {
      ...
      const vanillaJsButton = await page.waitForSelector("xpath///button[text() = 'Vanilla JS']");
  ...
  it.each(internalUrls.map<[string, number]>((url, i) => [url, i]))(
    ...
    async (url, index) => {
      ...
      const darkThemeButton = await page.waitForSelector("xpath///button[text() = 'Dark theme']");
