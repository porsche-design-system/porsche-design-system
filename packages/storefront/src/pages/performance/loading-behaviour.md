...
### Initialize Porsche Design System as early as possible
...
To achieve this bootstrapping earlier we provide a partial called `getLoader()` in all `@porsche-design-system/components-{js|angular|react}` packages which needs to be imported into the `<head>` of your `index.html`.
...
<head>
  ...
  <%= require('@porsche-design-system/components-{js|angular|react}/partials').getLoader() %>
</head>

<head>
  // without link tags  
  <script><%= require('@porsche-design-system/components-{js|angular|react}/partials').getLoader({ withoutTags: true }) %></script>
</head>
...
"scripts": {
  "prestart": "yarn replace",
  "replace": "placeholder='<!--PLACEHOLDER_PORSCHE_DESIGN_SYSTEM_LOADER-->' && partial=$placeholder$(node -e 'console.log(require(\"@porsche-design-system/components-js/partials\").getLoader())') && regex=$placeholder'.*' && sed -i '' -E -e \"s@$regex@$partial@\" index.html",
}
```
