...
**Important:** All elements are rendered on the browsers `#top-layer` so a manual ordering of z-index will have no
effect, instead the browser renders the element on top of everything which was triggered to be shown last,
