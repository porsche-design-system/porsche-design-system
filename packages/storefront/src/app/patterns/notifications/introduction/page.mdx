...
**Keep in mind:** Frequent or disruptive notifications can have a **negative impact** on the user's experience.
...
**Important:** All elements are rendered in the browser's `#top-layer`, so manual `z-index` ordering has no effect.
Instead, the browser renders the element that was triggered last on top of everything else,
...
- Kim Flaherty,
  [Indicators, Validations, and Notifications](https://nngroup.com/articles/indicators-validations-notifications)
