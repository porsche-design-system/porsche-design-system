...
export const metadata = {
  title: 'Initialization: Remix - Porsche Design System',
  description: 'Learn how to initialize and set up the Porsche Design System in your Remix app.',
};

# Initialization

<TableOfContents headings={['Setup', 'Optimization']} />
...
```tsx
<PorscheDesignSystemProvider>...</PorscheDesignSystemProvider>
```

When looking at Remix, we need to cover two different scenarios: the **server side** and the **client side**.
...
markup for something called [Declarative Shadow DOM](https://web.dev/declarative-shadow-dom) which is converted into a
real Shadow DOM by modern browsers without any JavaScript.
...
Now, we already have a `p-button` tag in the DOM from the server side. From here on the exact same things happen as
described at [Vanilla JS Preparation](/must-know/initialization/vanilla-js#preparation) and
