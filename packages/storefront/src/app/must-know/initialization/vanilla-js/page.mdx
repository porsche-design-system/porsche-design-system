...
export const metadata = {
  ...
  description: 'Learn how to initialize and set up the Porsche Design System in your app with Vanilla JS.',
};
...
First, we need to understand how a web component works before we can look at how to optimize its bootstrapping
behavior.
...
In the example above, we have not rendered any component yet, so let's add a single `p-button` tag to the DOM.

```html
<p-button>Hello</p-button>
```

For now, this is just a "dead" html tag without any appearance or functionality. To prevent Flash of Unstyled Content
(FOUC), the element needs to be hidden until it is registered or fully rendered. This can be done in multiple ways
depending on your setup and requirements.

Using the [`:defined`](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Selectors/:defined) CSS pseudo-class
which will match once the component has been successfully defined:

```html
p-button:not(:defined) { visibility: hidden; }
```

Using the Stencil [`hydrated`](https://stenciljs.com/docs/config#hydratedflag) class which will automatically be added
by Stencil once the component is fully rendered:

```html
p-button:not(.hydrated) { visibility: hidden; }
```
...
<Notification heading="Important" headingTag="h3" state="warning">
  It is important to understand, that for each component tag in the DOM, there is a component class instance in the
  JavaScript space. The component tag acts as an interface, similar to an API, where you set attributes/properties or
  children as inputs. If these change, the class instance detects these and renders the result into the component tag's
  Shadow DOM as an output.
</Notification>
...
- that injects the **component chunk**
...
<Notification heading="Hint" headingTag="h4" state="warning">
  This works especially well in a plain HTML and Vanilla JS setup, since the `index.js` would otherwise be bundled by the
  JavaScript framework and you would end up shipping the same code twice, once bundled and once inlined in the `script`
  tag.
</Notification>
