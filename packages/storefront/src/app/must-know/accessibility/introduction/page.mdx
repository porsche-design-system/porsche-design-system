...
Technically the Porsche Design System components are build out of **Web Components** to have a real
single-source-of–truth regardless in which Framework the component is used. Though Web Components are widely adopted,
there are still some **accessibility challenges**. One example is that it’s currently not possible to set `IDRef`
**relationships** due to the architecture of custom elements and their scoped shadowDOM. This represents a limitation in
**ARIA** support. However, there is an ongoing draft for a new **AOM**
([Accessibility Object Model](https://github.com/WICG/aom)), which aims to address this issue and enhance JS-based
accessibility support for all web components. We are continuously working to improve these existing limitations.
