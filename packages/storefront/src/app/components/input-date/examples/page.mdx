...
import { inputDateExample, inputDateExampleControlled } from '@porsche-design-system/shared/examples';
...
The `p-input-date` component is a form-associated custom element that integrates seamlessly with forms. Leveraging the
...
<ComponentExample codeSample={inputDateExample} />

## Controlled

In the controlled approach, the `p-input-date` value is externally managed. While the internal value will be updated
automatically, you can use the `input`, `change` or `blur` event to update the external state.

<ComponentExample codeSample={inputDateExampleControlled} />
