...
You can also use a custom wrapped label by just wrapping the `p-checkbox` component within a label tag. In this case,
make sure **not** to define the `label` property on the host element or use the named slot `label`. Be aware that by
