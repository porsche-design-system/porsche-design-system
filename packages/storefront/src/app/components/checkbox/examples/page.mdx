...
<TableOfContents headings={['Form', 'Indeterminate', 'Slots', 'Custom wrapped label (🧪Experimental)']} />
...
The `p-checkbox` component also supports a custom wrapped `label` by wrapping the component in a `label` tag.
...
clicked, call `event.preventDefault()` in the label’s click handler.
