...
This approach can be used if the `label` must be placed individually or besides other UI elements (e.g. `popover`). Be
aware that by using this approach, the **styling of the `label` must be controlled by the integrating team**.
