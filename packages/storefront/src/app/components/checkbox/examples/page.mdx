...
<TableOfContents headings={['Form', 'Indeterminate', 'Slots', 'Custom wrapped label']} />
...
Sometimes it's useful to be able to render markup (e.g. an anchor tag) for `label` or `message` or adding additional
markup (e.g. `popover`) at the end of a `label`. Therefore, a named slot can be used. Make sure **not** to define the
corresponding property on the host element when a named slot is used (because a property definition is preferred over a
named slot). For named slots only
...
contain raw text and optionally `a` or `button` elements. Be aware that by using this approach, the **styling of the
`label` must be controlled by the integrating team**.
