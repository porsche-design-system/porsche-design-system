...
import { inputSearchExample, inputSearchExampleControlled } from '@porsche-design-system/shared/examples';
...
The `p-input-search` component is a form-associated custom element that integrates seamlessly with forms. Leveraging the
...
<ComponentExample codeSample={inputSearchExample} />

## Controlled

In the controlled approach, the `p-input-search` value is externally managed. While the internal value will be updated
automatically, you can use the `input`, `change` or `blur` event to update the external state.

<ComponentExample codeSample={inputSearchExampleControlled} />
