...
<TableOfContents headings={['Form', 'Set Value', 'With optgroups', 'Custom asynchronous filtering']} />
...
To do so, place a `p-input-search` inside the `filter` slot and handle the user input and option fetching yourself.
Remember to also display “no results found” and error states as shown in the example.
