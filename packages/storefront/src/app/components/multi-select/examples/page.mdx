...
import {
  multiSelectStory,
  multiSelectStoryOptgroup,
  multiSelectStorySlots,
} from '@/app/components/multi-select/multi-select.stories';
...
In the **uncontrolled** approach, the select behaves similarly to a native `<select multiple>`, automatically managing its
...
<ComponentStory story={multiSelectStorySlots} />
