...
To make sure the user actively selects an option, set the `p-select` `value` property to `undefined` (or `null`) by
default.

If you want to allow users to clear their selection again, include an option without a value:

```
<p-select-option></p-select-option>
```

This option will be selected whenever the `p-select` `value` property is `undefined` or `null`.

If you don’t want users to clear their selection but still want to show a placeholder, add a disabled option instead:
