...
import { gridStoryArea, gridStoryDivision } from '@/app/(main)/tailwindcss/grid/examples/grid.stories';
...
divisions such as `one-half`, `one-third` (`basic` area only) and `two-thirds` (`basic` area only). When page sections
share one alignment system, apply `.grid-template` once to the page root and use `.col-full`, `.grid` and
`.grid-cols-subgrid` on each section. This avoids duplicate grid templates and keeps all sections aligned; section-level
templates remain valid for independent grids.
