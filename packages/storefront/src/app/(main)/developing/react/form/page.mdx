...
<WebsiteViewer sourceCodePath="frameworks/react" viewPath="react" title="React: Demo application" />
...
import { PInputText, type PInputTextInputEvent, type PInputTextProps } from '@porsche-design-system/components-react';
...
inference, typed refs, and the distinction between the host `target` and the event `detail`.
