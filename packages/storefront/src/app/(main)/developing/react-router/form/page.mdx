...
export function TextField() {
  ...
  return <PInputText name="myInputText" label="Some Label" value={value} onInput={onInput} />;
}
```

See [Typed Events and Element Refs](/developing/react/advanced#typed-events-and-element-refs) for inline handler
inference, typed refs, and the distinction between the host `target` and the event `detail`. Use the `/ssr` import path
for these examples in React Router SSR applications.
