...
focus shifts automatically to the next input, until every input is filled. Typing a digit into a filled input replaces
that digit and moves focus forward.

A few things to note:

- Only digits can be entered.
- Only one digit can be entered per input.
- Pressing a digit in a filled input overwrites the current value.
- `ArrowLeft` / `ArrowRight` move focus between inputs. `Home` / `End` jump to the first or last input.
