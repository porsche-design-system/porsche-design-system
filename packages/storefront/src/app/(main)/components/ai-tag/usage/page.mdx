...
- Use the appropriate `locale` to match the user's market (BCP47 e.g. `en-US` or POSIX e.g. `en_US`).
  Copy is resolved by language; markets without a translation fall back to English. Language-only `en` is supported
