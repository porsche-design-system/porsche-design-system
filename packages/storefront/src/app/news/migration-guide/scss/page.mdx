...
<TableOfContents headings={['SCSS', '👹Breaking Changes', 'Upcoming Changes']} />
...
- All new colors and styles are now controlled by CSS `color-scheme` using the CSS
  [`light-dark()`](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/color_value/light-dark) function.
  Therefore we provide the `@mixin color-scheme()` which includes utility classes including a polyfill for browsers not
  supporting the CSS `light-dark()`.
...
```html
<!-- Defined theme will be applied to all child elements and is also inherited through Shadow DOM -->
