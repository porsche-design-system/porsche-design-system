...
export const metadata = {
  title: 'Migration Guide: Utilities - Porsche Design System',
  description: 'Learn how to migrate from the utilities package to the new styles package.'
}

# Migration Guide

<TableOfContents headings={["Migrate from Utilities to Styles","👹 Breaking Changes (JS)","👹 Breaking Changes (SCSS)"]} />
...
<Notification heading="Deprecation hint" headingTag="h3" state="error">
  The npm package `@porsche-design-system/utilities` is deprecated and will no longer be maintained. A migration
  to `@porsche-design-system/components-{js|angular|react|vue}/styles` should take place as fast as possible.
</Notification>
