...
<Notification heading="Lightning CSS bug" heading-tag="h2" state="info">
  If you are using Vite v8 or later it relies on [`lightningcss`](https://lightningcss.dev/), which includes a polyfill
  for the `light-dark()` function that is currently **broken**. We recommend disabling the polyfill for now. [Learn
  more](/stylesheets/color-scheme/introduction/#lightning-css-bug)
</Notification>
