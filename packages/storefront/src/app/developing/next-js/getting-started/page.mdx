...
<Notification heading="Lightning CSS bug" heading-tag="h2" state="info">
  When using Next.js with Turbopack (default since v16) it relies on [`lightningcss`](https://lightningcss.dev/), which
  includes a polyfill for the `light-dark()` function that is currently **broken**. We recommend disabling the polyfill
  for now. [Learn more](/stylesheets/color-scheme/introduction/#lightning-css-bug)
</Notification>
