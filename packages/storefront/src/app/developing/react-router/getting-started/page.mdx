...
<Notification heading="Lightning CSS bug" heading-tag="h2" state="info">
  When using React Router with Vite v8 or later it relies on [`lightningcss`](https://lightningcss.dev/), which includes
  a polyfill for the `light-dark()` function that is currently **broken**. We recommend disabling the polyfill for now.
