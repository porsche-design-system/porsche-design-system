...
import { gridStoryArea, gridStoryGrid, gridStoryTiles } from '@/app/tailwind/grid/examples/grid.stories';
...
<TableOfContents headings={['Areas', 'Divisions']} />

The Porsche Design System Tailwind theme includes the custom `@utility` CSS class `.pds-grid` and CSS variables
`--pds-grid-*`, which together form a generic layout system called the "Porsche Grid". This system ensures consistent
content placement and alignment across multiple Porsche applications.
...
`.pds-grid` defines a new instance of the Porsche Grid. Although content placement is flexible across any CSS grid
...
divisions such as `one-half`, `one-third` and `two-thirds`.
