 ...
 *
 * Each skill calls this from its own `drift.spec.ts` so its snapshots land in that spec's
 * `__snapshots__` file: one skill's regeneration then never rewrites another's, and a skill's
 * snapshots stay reviewable next to the rest of its tests. The check itself is content-agnostic,
 * so it lives here once — `gates/skillSpecs.spec.ts` gates that every registered skill calls it.
 */
...
export const describeSkillTreeDrift = (skillId: SkillId): void => {
  describe(`${skillId} skill tree content`, () => {
