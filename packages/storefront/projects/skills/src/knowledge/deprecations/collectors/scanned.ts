...
import { partialsRoot, stylesheetsRoot, tokensMetaRoot } from './packageRoots';

/**
 * Collectors for the source categories that currently hold no deprecations: tokens, icons,
 * stylesheets and partials.
 *
 ...
 * declared empty but is not — the day tokens gain their first deprecation someone has to decide
...
export const collectTokenDeprecations = (): DeprecationSource =>
  scanned('tokens', '`@porsche-design-system/tokens` and its metadata', tokensMetaRoot());
