...
// stylis ships no bundled types; the build/test toolchain transpiles regardless.
// @ts-expect-error
...
type AstNode = { type: string; value: string; props: string | string[]; children: AstNode[] | string };
...
const printBlock = (header: string, el: AstNode, pad: string): string => {
  const decls = (el.children as AstNode[]).filter((c) => c.type === 'decl');
  if (!decls.length) return '';
  const inner = decls.map((d) => `${pad}  ${formatDecl(d)}`).join('\n');
  return `${pad}${header} {\n${inner}\n${pad}}\n`;
};

const printRule = (el: AstNode, pad: string): string => printBlock((el.props as string[]).join(', '), el, pad);

// @media/@supports block; children may be rules or nested conditionals
const printConditional = (query: string, children: AstNode[], pad = ''): string => {
  let body = '';
...
const compileBranch = (selector: string, subtree: any): AstNode[] =>
  compile(`${selector}{${serializeStyles([subtree]).styles}}`) as AstNode[];
...
export const getCss = (style: Styles): string => {
  ...
  const collect = (ast: AstNode[]): void => {
    for (const el of ast) {
      if (el.type === '@media') {
        ...
        mediaMap
          .get(el.value)!
          .push(...(el.children as AstNode[]).filter((c) => c.type === 'rule' || c.type === '@media' || c.type === '@supports'));
  ...
  for (const key in style) {
    if (key === '@global') {
      ...
      for (const face of Array.isArray(value) ? value : [value]) {
        collect(compile(`@font-face{${serializeStyles([face]).styles}}`) as AstNode[]);
      }
      ...
      collect([{ type, value: key, props: [key], children: inner }]);
    } else {
  ...
  for (const el of base) {
    ...
    else out += printConditional(el.value, el.children as AstNode[]);
  }
