...
const getComponentCss = (wrapContent, disablePagination, theme) => {
  ...
  return validateProps.getCss(Object.assign({ '@global': {
      ':host': validateProps.addImportantToEachRule({
        ...
        marginLeft: 'auto',
        marginRight: 'auto',
        gap: validateProps.pxToRemWithUnit(24),
        gridAutoFlow: 'row',
        [mediaQueryXl]: {
          gap: validateProps.pxToRemWithUnit(32),
        },
      }),
      'h2,::slotted([slot=heading])': validateProps.addImportantToEachRule(Object.assign(Object.assign({}, headingMediumStyle.headingMediumStyle), { margin: 0, color: primaryColor, maxWidth: validateProps.pxToRemWithUnit(900) })),
      'p,::slotted([slot=description])': validateProps.addImportantToEachRule(Object.assign(Object.assign({}, textSmallStyle.textSmallStyle), { margin: 0, color: primaryColor, maxWidth: validateProps.pxToRemWithUnit(550), gridColumn: 1 })),
    }, splide: {
      overflow: 'hidden',
      // visibility: 'hidden',
      '&__track': Object.assign(Object.assign({ cursor: 'grab' }, (wrapContent &&
        validateProps.addImportantToEachRule({
          ...
          padding: `0 calc(${gridSafeZone.gridSafeZone} + 7%) 0 ${gridSafeZone.gridSafeZone}`,
          [mediaQueryXl]: {
            padding: `0 calc(${gridSafeZone.gridSafeZone} + 7%) 0 ${gridSafeZone.gridSafeZone}`,
          },
        }))), { '&--draggable': {
      ...
      '&__list': Object.assign({ display: 'flex', height: '100%' }, validateProps.getBackfaceVisibilityJssStyle()),
      '&__slide': Object.assign(Object.assign({ position: 'relative', flexShrink: 0 }, validateProps.getBackfaceVisibilityJssStyle()), { transform: 'translateZ(0)' }),
      '&__sr': validateProps.getScreenReaderOnlyJssStyle(), // appears in the DOM when sliding
    }, 
    ...
    header: {
      display: 'grid',
      gap: gridGap,
      padding: wrapContent ? `0 ${gridSafeZone.gridSafeZone}` : null,
      [mediaQueryS]: {
        gridTemplateColumns: `minmax(0px, 1fr) ${validateProps.pxToRemWithUnit(80)}`,
        position: 'relative',
        minHeight: validateProps.pxToRemWithUnit(40), // actual height of prev/next buttons
      },
    }, nav: {
      display: 'none',
      [mediaQueryS]: {
        ...
        gridAutoFlow: 'column',
        position: 'absolute',
        right: wrapContent ? gridSafeZone.gridSafeZone : 0,
        bottom: 0,
      },
    }, btn: {
      padding: validateProps.pxToRemWithUnit(8),
    } }, (disablePagination !== true && {
    pagination: Object.assign(Object.assign({}, validateProps.buildResponsiveStyles(disablePagination, (value) => ({ display: value ? 'none' : 'grid' }))), { gridAutoColumns: validateProps.pxToRemWithUnit(8), gridAutoFlow: 'column', justifyContent: 'center', gap: validateProps.pxToRemWithUnit(8), height: validateProps.pxToRemWithUnit(8) }),
    bullet: {
      borderRadius: validateProps.pxToRemWithUnit(4),
...
const getSplideBreakpoints = (perPage, gap) => {
  return validateProps.mergeDeep(toSplideBreakpoints('perPage', perPage), toSplideBreakpoints('gap', gap));
};
const toSplideBreakpoints = (propName, value) => {
  return typeof value === 'object'
    ? Object.entries(value).reduce((result, [key, val]) => (Object.assign(Object.assign({}, result), { 
      // cut off 'px' suffix
      [key === 'base' ? 0 : validateProps.breakpoint[key].slice(0, -2)]: {
    ...
    : {
      0: {
        // round to sanitize floating numbers
        [propName]: propName === 'perPage' ? Math.round(value) : value,
      },
    };
};
const warnIfHeadingIsMissing = (host, heading) => {
  if (!heading && !hasNamedSlot.hasNamedSlot(host, 'heading')) {
    console.warn(`A heading has to be set via property or named slot on ${validateProps.getTagName(host)} in order to ensure accessibility.`);
  }
};
const getSlidesAndAddNamedSlots = (host) => {
...
const updatePagination = (paginationEl, newIndex) => {
  if (paginationEl) {
...
let hasInertSupport = typeof HTMLElement !== 'undefined' && HTMLElement.prototype.hasOwnProperty('inert');
const updateSlidesInert = (splide) => {
  ...
  if (splide) {
    ...
    const { index, options: { perPage }, } = splide;
    const maxIndex = index + perPage;
    if (hasInertSupport) {
      // add inert attribute on slides in shadowDOM
      // https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/inert
      // https://caniuse.com/?search=inert
      slides.forEach((slide, i) => i >= index && i < maxIndex ? slide.removeAttribute('inert') : slide.setAttribute('inert', ''));
    }
    else {
      // fallback with tabindex handling for certain elements in lightDOM
      const prefix = validateProps.getTagName(splide.root.getRootNode().host).replace('carousel', '');
      const tagNames = ['p-button', 'p-button-pure', 'p-link', 'p-link-pure'];
      const pdsSelectors = tagNames.map((tagName) => tagName.replace(/^p-/, prefix)).join();
      slides.forEach((slide, i) => slide.firstChild.assignedNodes()[0]
        .querySelectorAll(`[href],button,${pdsSelectors}`)
        .forEach((el) => i >= index && i < maxIndex ? el.removeAttribute('tabindex') : el.setAttribute('tabindex', '-1')));
    }
  }
};
...
const propTypes = {
  ...
  slidesPerPage: validateProps.AllowedTypes.breakpoint('number'),
...
const Carousel = class {
  constructor(hostRef) {
    ...
    this.updateAmountOfPages = () => {
      ...
      // round to sanitize floating numbers
      Math.round(validateProps.getCurrentMatchingBreakpointValue(this.slidesPerPage)));
      renderPagination(this.pagination, this.amountOfPages, ((_a = this.splide) === null || _a === void 0 ? void 0 : _a.index) || 0);
  ...
  componentDidLoad() {
    this.splide = new Splide(this.container, {
      ...
      breakpoints: getSplideBreakpoints(this.slidesPerPage, {
        base: spacingStaticSmall.spacingStaticSmall,
        s: spacingStaticMedium.spacingStaticMedium,
        l: spacingStaticLarge.spacingStaticLarge,
      }),
      // https://splidejs.com/guides/i18n/#default-texts
      i18n: validateProps.parseJSONAttribute(this.intl),
    });
    this.registerSplideHandlers(this.splide);
  }
  componentDidUpdate() {
  ...
  render() {
    ...
    validateProps.attachComponentCss(this.host, getComponentCss, this.wrapContent, this.disablePagination, this.theme);
    ...
    return (validateProps.h(validateProps.Host, null, validateProps.h("div", { class: "header" }, this.heading ? validateProps.h("h2", null, this.heading) : validateProps.h("slot", { name: "heading" }), hasDescription.hasDescription(this.host, this.description) &&
      ((this.description && validateProps.h("p", null, this.description)) || validateProps.h("slot", { name: "description" })), validateProps.h("div", { class: "nav" }, validateProps.h(PrefixedTagNames.pButtonPure, Object.assign({}, btnProps, { icon: "arrow-left", ref: (ref) => (this.btnPrev = ref), onClick: () => slidePrev(this.splide, this.amountOfPages) })), validateProps.h(PrefixedTagNames.pButtonPure, Object.assign({}, btnProps, { icon: "arrow-right", ref: (ref) => (this.btnNext = ref), onClick: () => slideNext(this.splide, this.amountOfPages) })))), validateProps.h("div", { id: "splide", class: "splide", "aria-label": this.heading || getSlotTextContent.getSlotTextContent(this.host, 'heading'), ref: (ref) => (this.container = ref) }, validateProps.h("div", { class: "splide__track" }, validateProps.h("div", { class: "splide__list" }, this.slides.map((_, i) => (validateProps.h("div", { key: i, class: "splide__slide" }, validateProps.h("slot", { name: `slide-${i}` }))))))), this.disablePagination !== true && validateProps.h("div", { class: "pagination", ref: (ref) => (this.pagination = ref) })));
  }
