...
![](%E2%9A%92%20Basics:%20Typography/44A66743-79D1-4044-90E3-9C26914A5563.png)
...
❌
*@DEV:  Die jeweilige Zahl sollten wir dann in der entsprechenden Größe abbilden.*
❌
...
❌ ABBILDUNG FOLGT: BEISPIEL FÜR HEADLINE ❌
...
❌ ABBILDUNG FOLGT: BEISPIEL HEADLINE-SUBLINE-KOMBI ❌
...
❌ ABBILDUNG FOLGT: STATES IN LIGHT & DARK THEME ❌ 
...
❌ ABBILDUNG FOLGT: BEISPIEL ADDITIONAL SIZES ❌
...
![](%E2%9A%92%20Basics:%20Typography/7FF6159E-1635-4B5E-83FA-87FEABAE48C3.png)
...
The vertical spacing between two copy text paragraphs should be at least 24 px, which equals the space of one line of text in-between.

❌ ABBILDUNG FOLGT: PARAGRAPH SPACING – BOUNDING BOX TO BOUNDING BOX ❌
...
## Accessibility
In order to make content accessible for a wide range of people and, by doing so, to improve the product’s usability in general, we stick to the common [WCAG 2.1 Standard](https://ui.porsche.com/latest/#/basics/accessibility). when designing Porsche web applications.

- When designing with text, always ensure a sufficient contrast ratio. The combination of text and background color should pass the WCAG AA standard and have a contrast ratio of at least 4.5:1 for standard text size and 3:1 for larger text sizes. 

❌  ABBILDUNG FOLGT: CONTRAST RATIOS (sh. Color Doku) ❌

- When communicating element states with text (for example a password validation information in forms), keep an eye on not rely on color only to convey information, especially in critical cases, e.g. when displaying safety-relevant vehicle data. Use additional text stylings like different font weights (sticking to the general Porsche rules for text styling as described above) to make it easy for visually impaired people to recognise status information even if they can’t see the color.

❌ *@MARCEL: Würde diesen Part nochmal mit der Color Doku abgleichen und dort evtl. teilweise rausnehmen bzw. referenzieren, um Redundanzen zu vermeiden.* ❌
