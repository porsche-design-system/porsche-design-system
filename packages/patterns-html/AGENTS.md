...
Two cheaper migration targets are already implemented and measured: [`packages/patterns-nunjucks`](../patterns-nunjucks)
(template engine) and [`packages/patterns-jsx`](../patterns-jsx) (TSX components rendered to static HTML). Both produce
structurally identical `dist/` output. Read [`COMPARISON.md`](../patterns-nunjucks/COMPARISON.md) before proposing
either an engine extension or a migration.
