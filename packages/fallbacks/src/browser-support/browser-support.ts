...
/* Auto Generated Start */
  ...
  const htmlMarkup = `<strong>${title}</strong><p>${content.replace('<br>','<br class=show--at-768-ilb>')}</p><div class=show--at-768>${[chrome, firefox, edge].map((link) => link.replace('>', '><i></i>')).join('')}</div>`;
