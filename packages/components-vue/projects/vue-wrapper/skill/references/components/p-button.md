...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-button/examples/Default.vue](./p-button/examples/Default.vue) |
| Form | When used as a submit button, the `name` and `value` props are submitted as a pair as part of the form data. | [./p-button/examples/Form.vue](./p-button/examples/Form.vue) |
| Form Attribute | When a button is used as a submit or reset button outside a form, the `form` attribute can be utilized to explicitly associate the button with a specific form element. | [./p-button/examples/FormAttribute.vue](./p-button/examples/FormAttribute.vue) |
