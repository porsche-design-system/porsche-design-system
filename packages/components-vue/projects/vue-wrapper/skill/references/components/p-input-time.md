...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-input-time/examples/Default.vue](./p-input-time/examples/Default.vue) |
| Form | The `p-input-time` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-input-time/examples/Form.vue](./p-input-time/examples/Form.vue) |
| Slots | Use named slots when `label`, `description` or `message` need markup such as a link. | [./p-input-time/examples/Slots.vue](./p-input-time/examples/Slots.vue) |
