...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-input-number/examples/Default.vue](./p-input-number/examples/Default.vue) |
| Form | The `p-input-number` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-input-number/examples/Form.vue](./p-input-number/examples/Form.vue) |
| Slots | Use named slots when `label`, `description` or `message` need markup such as a link. | [./p-input-number/examples/Slots.vue](./p-input-number/examples/Slots.vue) |
