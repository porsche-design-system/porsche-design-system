...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-divider/examples/Default.vue](./p-divider/examples/Default.vue) |
| Vertical | **Hint:** The component has not an implicit vertical height unless we set a container which provides it or define the height on the component itself. | [./p-divider/examples/Vertical.vue](./p-divider/examples/Vertical.vue) |
| Responsive | You can change the divider's direction by using different major breakpoints `xs`, `s`, `m`, `l`, `xl`. | [./p-divider/examples/Responsive.vue](./p-divider/examples/Responsive.vue) |
