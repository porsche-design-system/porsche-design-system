...
// One record per icon, listed in p-icon.angular.figma.batch.json; a parent template reads the swapped icon as
// instance.getInstanceSwap('icon').executeTemplate().metadata.props.name
import figma from 'figma';

export default {
  example: figma.code`<p-icon [name]="'${figma.batch.name}'"></p-icon>`,
  imports: [
