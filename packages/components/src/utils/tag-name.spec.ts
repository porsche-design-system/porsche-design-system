...
describe('getPrefixedTagNames()', () => {
  ...
  it('should return an object with a mapping of all tag names to the prefixed ones', () => {
    ...
    expect(Object.keys(resultWithoutPrefix).length).toEqual(TAG_NAMES.length - 2); // p-text and p-headline
    ...
    expect(Object.keys(resultWithPrefix).length).toEqual(TAG_NAMES.length - 2); // p-text and p-headline
  });
