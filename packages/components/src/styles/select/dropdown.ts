...
export const getSelectDropdownKeyframesStyles: Styles = {
...
export const getSelectDropdownJssStyle = (
  isOpen: boolean,
  ...
  const minHeightOptionList = `calc(${4.5 * (optionHeight + 8) + 6 + 2 + (hasFilter ? 54 : 0)}px)`; // 4.5 options * option height + 8px gap + additional spacing (6px = padding, 2px = border)
