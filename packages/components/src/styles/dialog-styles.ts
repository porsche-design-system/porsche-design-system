...
export const getDialogDismissButtonJssStyle = (): JssStyle => {
  return {
    ...
    gridArea: '1/3',
    zIndex: 4, // controls layering + creates new stacking context (prevents content within to be above other dialog areas)
    position: 'sticky',
...
export const getSlotHeaderJssStyle = (): JssStyle => {
  ...
  return {
    ...
    background: `linear-gradient(180deg,${canvasColor} 0%,${canvasColor} 80%,transparent 100%)`,
  };
};
...
export const getSlotFooterJssStyle = (): JssStyle => {
  ...
  return {
    ...
    background: `linear-gradient(0deg,${canvasColor} 0%,${canvasColor} 20%,transparent 80%)`,
    '&[data-stuck]::after': {
      content: '""',
      zIndex: -1,
      position: 'absolute',
