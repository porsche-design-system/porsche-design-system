...
export default {
  example: figma.code`<p-inline-notification${heading ? ` [heading]="'${heading}'"` : ''}${headingTag ? ` [headingTag]="'${headingTag}'"` : ''} [state]="'${state}'"${dismissButton ? ' [dismissButton]="true"' : ''}${actionLabel ? ` [actionLabel]="'${actionLabel}'"` : ''}${actionIcon ? ` [actionIcon]="'${actionIcon}'"` : ''}>${slotDefault}</p-inline-notification>`,
  imports: [
