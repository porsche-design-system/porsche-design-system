...
export default {
  example: figma.code`<p-input-password${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${compact ? ' compact="true"' : ''}${value ? ` value="${value}"` : ''}${readOnly ? ' read-only="true"' : ''}${placeholder ? ` placeholder="${placeholder}"` : ''}${disabled ? ' disabled="true"' : ''}${required ? ' required="true"' : ''}${loading ? ' loading="true"' : ''} state="${state}"${message ? ` message="${message}"` : ''}${hideLabel ? ' hide-label="true"' : ''}${toggle ? ' toggle="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotStart, 'slot-start', '<!-- slot="start" -->')}${slotted(slotEnd, 'slot-end', '<!-- slot="end" -->')}</p-input-password>`,
  imports: ['<!-- Docs: https://designsystem.porsche.com/v4/components/input-password/api -->'],
  id: 'p-input-password',
