...
export default {
  example: figma.code`<p-input-password${loading ? ' loading="true"' : ''}${placeholder ? ` placeholder="${placeholder}"` : ''}${toggle ? ' toggle="true"' : ''}${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${message ? ` message="${message}"` : ''}${required ? ' required="true"' : ''}${value ? ` value="${value}"` : ''} state="${state}"${compact ? ' compact="true"' : ''}${disabled ? ' disabled="true"' : ''}${readOnly ? ' read-only="true"' : ''}${hideLabel ? ' hide-label="true"' : ''}>${slotted(slotStart, 'slot-start', '<!-- slot="start" -->')}${slotted(slotEnd, 'slot-end', '<!-- slot="end" -->')}${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}</p-input-password>`,
