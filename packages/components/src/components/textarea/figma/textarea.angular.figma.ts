...
export default {
  example: figma.code`<p-textarea${label ? ` [label]="'${label}'"` : ''}${description ? ` [description]="'${description}'"` : ''}${compact ? ' [compact]="true"' : ''}${value ? ` [value]="'${value}'"` : ''} [state]="'${state}'"${message ? ` [message]="'${message}'"` : ''}${hideLabel ? ' [hideLabel]="true"' : ''}${required ? ' [required]="true"' : ''}${disabled ? ' [disabled]="true"' : ''}${readOnly ? ' [readOnly]="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}</p-textarea>`,
  imports: [
