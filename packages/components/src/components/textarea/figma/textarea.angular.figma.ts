...
export default {
  example: figma.code`<p-textarea${hideLabel ? ' [hideLabel]="true"' : ''}${value ? ` [value]="'${value}'"` : ''}${label ? ` [label]="'${label}'"` : ''}${description ? ` [description]="'${description}'"` : ''}${message ? ` [message]="'${message}'"` : ''}${required ? ' [required]="true"' : ''} [state]="'${state}'"${compact ? ' [compact]="true"' : ''}${disabled ? ' [disabled]="true"' : ''}${readOnly ? ' [readOnly]="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}</p-textarea>`,
  imports: [
