...
export default {
  example: figma.code`<p-button${disabled ? ' disabled="true"' : ''}${loading ? ' loading="true"' : ''} variant="${variant}"${icon ? ` icon="${icon}"` : ''}${hideLabel ? ' hide-label="true"' : ''}${compact ? ' compact="true"' : ''}>${slotDefault}</p-button>`,
