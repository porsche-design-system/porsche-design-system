...
export default {
  example: figma.code`<p-link${icon ? ` icon="${icon}"` : ''} variant="${variant}"${hideLabel ? ' hide-label="true"' : ''}${compact ? ' compact="true"' : ''}>${slotDefault}</p-link>`,
  imports: ['<!-- Docs: https://designsystem.porsche.com/v4/components/link/api -->'],
  id: 'p-link',
