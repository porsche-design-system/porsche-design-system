...
export default {
  example: figma.code`<p-pin-code${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${hideLabel ? ' hide-label="true"' : ''} state="${state}"${disabled ? ' disabled="true"' : ''}${loading ? ' loading="true"' : ''}${required ? ' required="true"' : ''}${message ? ` message="${message}"` : ''} type="${type}"${compact ? ' compact="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}</p-pin-code>`,
  imports: ['<!-- Docs: https://designsystem.porsche.com/v4/components/pin-code/api -->'],
  id: 'p-pin-code',
