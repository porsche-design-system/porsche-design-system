...
export default {
  example: figma.code`<p-pin-code${hideLabel ? ' hide-label="true"' : ''}${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${message ? ` message="${message}"` : ''}${required ? ' required="true"' : ''} type="${type}" state="${state}"${disabled ? ' disabled="true"' : ''}${loading ? ' loading="true"' : ''}${compact ? ' compact="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}</p-pin-code>`,
