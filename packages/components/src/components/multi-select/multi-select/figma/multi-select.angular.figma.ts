...
export default {
  example: figma.code`<p-multi-select${hideLabel ? ' [hideLabel]="true"' : ''}${label ? ` [label]="'${label}'"` : ''}${description ? ` [description]="'${description}'"` : ''}${message ? ` [message]="'${message}'"` : ''}${required ? ' [required]="true"' : ''}${value ? ` [value]="'${value}'"` : ''} [state]="'${state}'" [dropdownDirection]="'${dropdownDirection}'"${compact ? ' [compact]="true"' : ''}${disabled ? ' [disabled]="true"' : ''}>${slotted(slotFilter, 'slot-filter', '<!-- slot="filter" -->')}${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotDefault, 'slot-default')}</p-multi-select>`,
  imports: [
