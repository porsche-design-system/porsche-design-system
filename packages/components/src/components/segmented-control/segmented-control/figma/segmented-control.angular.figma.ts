...
export default {
  example: figma.code`<p-segmented-control${label ? ` [label]="'${label}'"` : ''}${hideLabel ? ' [hideLabel]="true"' : ''}${required ? ' [required]="true"' : ''}${description ? ` [description]="'${description}'"` : ''}${message ? ` [message]="'${message}'"` : ''} [state]="'${state}'"${noWrap ? ' [noWrap]="true"' : ''}${disabled ? ' [disabled]="true"' : ''}${compact ? ' [compact]="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotDefault, 'slot-default')}</p-segmented-control>`,
  imports: [
