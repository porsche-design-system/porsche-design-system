...
export default {
  example: figma.code`<p-segmented-control${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${compact ? ' compact="true"' : ''} state="${state}"${required ? ' required="true"' : ''}${message ? ` message="${message}"` : ''}${hideLabel ? ' hide-label="true"' : ''}${disabled ? ' disabled="true"' : ''}${noWrap ? ' no-wrap="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotDefault, 'slot-default')}</p-segmented-control>`,
