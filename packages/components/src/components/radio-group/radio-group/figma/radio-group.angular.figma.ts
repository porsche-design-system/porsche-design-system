...
export default {
  example: figma.code`<p-radio-group${label ? ` [label]="'${label}'"` : ''}${description ? ` [description]="'${description}'"` : ''}${compact ? ' [compact]="true"' : ''} [direction]="'${direction}'"${disabled ? ' [disabled]="true"' : ''}${required ? ' [required]="true"' : ''}${loading ? ' [loading]="true"' : ''} [state]="'${state}'"${message ? ` [message]="'${message}'"` : ''}${hideLabel ? ' [hideLabel]="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotDefault, 'slot-default')}</p-radio-group>`,
  imports: [
