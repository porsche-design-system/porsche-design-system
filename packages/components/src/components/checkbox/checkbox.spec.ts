...
describe('componentDidRender', () => {
  ...
  it('should attach click event listener to external label when not already attached', () => {
    const component = initComponent();
    const externalLabel = document.createElement('label');
    document.body.appendChild(externalLabel);
    externalLabel.appendChild(component.host);
    ...
    component['externalLabel'] = externalLabel;
    component.label = '';
    component['isExternalLabelClickListenerAttached'] = false;

    const addEventListenerSpy = vi.spyOn(externalLabel, 'addEventListener');

    component.componentDidRender();

    expect(addEventListenerSpy).toHaveBeenCalledWith('click', component['onExternalLabelClick'], true);
    expect(component['isExternalLabelClickListenerAttached']).toBe(true);

    // Cleanup
    document.body.removeChild(externalLabel);
  });

  it('should not attach click event listener if already attached', () => {
    const component = initComponent();
    const externalLabel = document.createElement('label');
    document.body.appendChild(externalLabel);
    externalLabel.appendChild(component.host);
    ...
    component['externalLabel'] = externalLabel;
    component.label = '';
    component['isExternalLabelClickListenerAttached'] = true;
  ...
  it('should not attach click event listener when internal label prop is provided', () => {
    const component = initComponent();
    const externalLabel = document.createElement('label');
    document.body.appendChild(externalLabel);
    externalLabel.appendChild(component.host);
    ...
    component['externalLabel'] = externalLabel;
    component.label = 'Internal Label';
    ...
    expect(addEventListenerSpy).not.toHaveBeenCalled();
    expect(component['isExternalLabelClickListenerAttached']).toBe(false);

    // Cleanup
    document.body.removeChild(externalLabel);
  });

  it('should not attach click event listener when no external label exists', () => {
    const component = initComponent();
    component['externalLabel'] = null;
    component.label = '';
  ...
  it('should handle multiple renders without attaching listener multiple times', () => {
    const component = initComponent();
    const externalLabel = document.createElement('label');
    document.body.appendChild(externalLabel);
    externalLabel.appendChild(component.host);
    ...
    // Second render
    component.componentDidRender();
    expect(addEventListenerSpy).toHaveBeenCalledTimes(1); // Still only called once
...
describe('onExternalLabelClick', () => {
  it('should call preventDefault and click on checkboxInputElement if target is not interactive', () => {
    const component = initComponent();
  ...
  it('should call preventDefault but not click checkbox if component is disabled', () => {
    const component = initComponent();
    component.disabled = true;
    const clickSpy = vi.spyOn(component['checkboxInputElement'], 'click');
    const preventDefaultSpy = vi.fn();
  ...
  it('should call preventDefault but not click checkbox if component is loading', () => {
    const component = initComponent();
    component.loading = true;
    const clickSpy = vi.spyOn(component['checkboxInputElement'], 'click');
    const preventDefaultSpy = vi.fn();
  ...
  it('should do nothing if target is a "button" tag', () => {
    const component = initComponent();
    const clickSpy = vi.spyOn(component['checkboxInputElement'], 'click');
    const preventDefaultSpy = vi.fn();
    const event = {
