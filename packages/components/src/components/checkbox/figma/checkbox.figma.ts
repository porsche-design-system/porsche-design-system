...
export default {
  example: figma.code`<p-checkbox${required ? ' required="true"' : ''}${disabled ? ' disabled="true"' : ''}${indeterminate ? ' indeterminate="true"' : ''}${checked ? ' checked="true"' : ''}${label ? ` label="${label}"` : ''} state="${state}"${message ? ` message="${message}"` : ''}${hideLabel ? ' hide-label="true"' : ''}${loading ? ' loading="true"' : ''}${compact ? ' compact="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}</p-checkbox>`,
