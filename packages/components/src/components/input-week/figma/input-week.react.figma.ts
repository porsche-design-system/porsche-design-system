...
export default {
  example: figma.code`<PInputWeek${loading ? ' loading={true}' : ''}${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${message ? ` message="${message}"` : ''}${required ? ' required={true}' : ''}${value ? ` value="${value}"` : ''} state="${state}"${compact ? ' compact={true}' : ''}${disabled ? ' disabled={true}' : ''}${readOnly ? ' readOnly={true}' : ''}${hideLabel ? ' hideLabel={true}' : ''}>${slotted(slotStart, 'slot-start', '{/* slot="start" */}')}${slotted(slotEnd, 'slot-end', '{/* slot="end" */}')}${slotted(slotLabelAfter, 'slot-label-after', '{/* slot="label-after" */}')}</PInputWeek>`,
  imports: [
    '// Docs: https://designsystem.porsche.com/v4/components/input-week/api',
