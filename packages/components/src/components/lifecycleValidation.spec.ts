...
// TODO: p-select-wrapper and p-select-wrapper-dropdown is ignored since it behaved different as an internal component
describe.each<TagName>(
  tagNamesWithObserveChildren.filter(
    (tagName) => tagName !== 'p-select-wrapper-dropdown' && tagName !== 'p-select-wrapper'
  )
)('%s', (tagName) => {
  const component = componentFactory(tagName);
