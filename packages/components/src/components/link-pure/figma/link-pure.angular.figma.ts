...
export default {
  example: figma.code`<p-link-pure [alignLabel]="'${alignLabel}'"${stretch ? ' [stretch]="true"' : ''} [color]="'${color}'"${icon ? ` [icon]="'${icon}'"` : ''}${active ? ' [active]="true"' : ''}${hideLabel ? ' [hideLabel]="true"' : ''}></p-link-pure>`,
  imports: [
