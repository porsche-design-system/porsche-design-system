...
export default {
  example: figma.code`<p-link-pure${icon ? ` [icon]="'${icon}'"` : ''} [alignLabel]="'${alignLabel}'"${active ? ' [active]="true"' : ''}${stretch ? ' [stretch]="true"' : ''}${hideLabel ? ' [hideLabel]="true"' : ''} [color]="'${color}'"></p-link-pure>`,
  imports: [
