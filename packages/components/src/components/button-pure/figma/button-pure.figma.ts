...
export default {
  example: figma.code`<p-button-pure${icon ? ` icon="${icon}"` : ''} align-label="${alignLabel}"${disabled ? ' disabled="true"' : ''}${loading ? ' loading="true"' : ''}${active ? ' active="true"' : ''}${stretch ? ' stretch="true"' : ''}${hideLabel ? ' hide-label="true"' : ''} color="${color}"></p-button-pure>`,
