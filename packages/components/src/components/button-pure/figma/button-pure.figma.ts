...
export default {
  example: figma.code`<p-button-pure${disabled ? ' disabled="true"' : ''}${loading ? ' loading="true"' : ''} color="${color}"${icon ? ` icon="${icon}"` : ''}${active ? ' active="true"' : ''}${hideLabel ? ' hide-label="true"' : ''} align-label="${alignLabel}"${stretch ? ' stretch="true"' : ''}></p-button-pure>`,
  imports: ['<!-- Docs: https://designsystem.porsche.com/v4/components/button-pure/api -->'],
  id: 'p-button-pure',
