...
export default {
  example: figma.code`<p-button-pure${icon ? ` [icon]="'${icon}'"` : ''} [alignLabel]="'${alignLabel}'"${disabled ? ' [disabled]="true"' : ''}${loading ? ' [loading]="true"' : ''}${active ? ' [active]="true"' : ''}${stretch ? ' [stretch]="true"' : ''}${hideLabel ? ' [hideLabel]="true"' : ''} [color]="'${color}'"></p-button-pure>`,
  imports: [
