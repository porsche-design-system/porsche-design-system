...
export default {
  example: figma.code`<p-button-pure${disabled ? ' [disabled]="true"' : ''}${loading ? ' [loading]="true"' : ''} [color]="'${color}'"${icon ? ` [icon]="'${icon}'"` : ''}${active ? ' [active]="true"' : ''}${hideLabel ? ' [hideLabel]="true"' : ''} [alignLabel]="'${alignLabel}'"${stretch ? ' [stretch]="true"' : ''}></p-button-pure>`,
  imports: [
