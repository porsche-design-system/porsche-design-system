...
If the Marque is used with a link, make sure to add a descriptive label to inform screen reader users what the link stands for. This can be done through **ARIA** with the `accessibility` property.
...
<script lang="ts">
  ...
  export default class Code extends Vue {
    ...
    marqueWithTMAndLink = `<p-marque href="https://www.porsche.com" accessibility="{ 'aria-label': 'Porsche Homepage' }"></p-marque>`;
