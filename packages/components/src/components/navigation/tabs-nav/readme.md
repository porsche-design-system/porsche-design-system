# Tabs



<!-- Auto Generated Below -->


## Properties

| Property         | Attribute          | Description                                                   | Type                                                                                                                                                                      | Default     |
| ---------------- | ------------------ | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `activeTabIndex` | `active-tab-index` | Defines which tab to be visualized as selected.               | `number`                                                                                                                                                                  | `0`         |
| `colorScheme`    | `color-scheme`     | Adapts the background gradient color of prev and next button. | `"default" \| "surface"`                                                                                                                                                  | `'default'` |
| `size`           | `size`             | The text size.                                                | `string \| { base: "small" \| "medium"; xs?: "small" \| "medium"; s?: "small" \| "medium"; m?: "small" \| "medium"; l?: "small" \| "medium"; xl?: "small" \| "medium"; }` | `'medium'`  |
| `theme`          | `theme`            | Adapts color when used on dark background.                    | `"dark" \| "light"`                                                                                                                                                       | `'light'`   |
| `weight`         | `weight`           | The text weight.                                              | `"regular" \| "semibold"`                                                                                                                                                 | `'regular'` |


## Dependencies

### Depends on

- [p-button-pure](../../action/button-pure)