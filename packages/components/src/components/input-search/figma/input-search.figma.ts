...
export default {
  example: figma.code`<p-input-search${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${compact ? ' compact="true"' : ''}${value ? ` value="${value}"` : ''}${clear ? ' clear="true"' : ''}${indicator ? ' indicator="true"' : ''}${readOnly ? ' read-only="true"' : ''}${placeholder ? ` placeholder="${placeholder}"` : ''}${disabled ? ' disabled="true"' : ''}${required ? ' required="true"' : ''}${loading ? ' loading="true"' : ''} state="${state}"${message ? ` message="${message}"` : ''}${hideLabel ? ' hide-label="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotStart, 'slot-start', '<!-- slot="start" -->')}${slotted(slotEnd, 'slot-end', '<!-- slot="end" -->')}</p-input-search>`,
  imports: ['<!-- Docs: https://designsystem.porsche.com/v4/components/input-search/api -->'],
  id: 'p-input-search',
