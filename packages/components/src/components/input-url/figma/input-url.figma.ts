...
export default {
  example: figma.code`<p-input-url${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${compact ? ' compact="true"' : ''}${value ? ` value="${value}"` : ''}${readOnly ? ' read-only="true"' : ''}${placeholder ? ` placeholder="${placeholder}"` : ''}${disabled ? ' disabled="true"' : ''}${required ? ' required="true"' : ''}${loading ? ' loading="true"' : ''} state="${state}"${indicator ? ' indicator="true"' : ''}${message ? ` message="${message}"` : ''}${hideLabel ? ' hide-label="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotStart, 'slot-start', '<!-- slot="start" -->')}${slotted(slotEnd, 'slot-end', '<!-- slot="end" -->')}</p-input-url>`,
  imports: ['<!-- Docs: https://designsystem.porsche.com/v4/components/input-url/api -->'],
  id: 'p-input-url',
