...
const initComponent = (): InputEmail => {
  const component = new InputEmail();
  component.host = document.createElement('p-input-email');
...
describe('componentWillLoad', () => {
  ...
  it('should keep an existing string value untouched and store it as defaultValue', () => {
    const component = initComponent();
    component.value = 'a@b.de';
    component.componentWillLoad();

    expect(component.value).toBe('a@b.de');
    expect(component['defaultValue']).toBe('a@b.de');
  });
});
...
describe('onValueChange (inputElement sync)', () => {
  ...
  it('should sync inputElement.value with the coerced string', () => {
    ...
    component.value = 'a@b.de';
    component.onValueChange();
    expect(component['inputElement'].value).toBe('a@b.de');
...
describe('componentDidRender (setFormValue coercion to string)', () => {
  ...
  it('should call setFormValue with the new string value', () => {
    ...
    component.value = 'a@b.de';
    component.componentDidRender();
    expect(setFormValueSpy).toHaveBeenCalledWith('a@b.de');
  });
});
