...
export default {
  example: figma.code`<p-select${hideLabel ? ' hide-label="true"' : ''}${filter ? ' filter="true"' : ''}${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${message ? ` message="${message}"` : ''}${required ? ' required="true"' : ''}${value ? ` value="${value}"` : ''} state="${state}" dropdown-direction="${dropdownDirection}"${compact ? ' compact="true"' : ''}${disabled ? ' disabled="true"' : ''}>${slotted(slotFilter, 'slot-filter', '<!-- slot="filter" -->')}${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotDefault, 'slot-default')}</p-select>`,
  imports: ['<!-- Docs: https://designsystem.porsche.com/v4/components/select/api -->'],
  id: 'p-select',
