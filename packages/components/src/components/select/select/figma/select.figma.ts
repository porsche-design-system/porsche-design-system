...
export default {
  example: figma.code`<p-select${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${value ? ` value="${value}"` : ''} state="${state}"${message ? ` message="${message}"` : ''}${hideLabel ? ' hide-label="true"' : ''}${disabled ? ' disabled="true"' : ''}${required ? ' required="true"' : ''} dropdown-direction="${dropdownDirection}"${filter ? ' filter="true"' : ''}${compact ? ' compact="true"' : ''}>${slotted(slotLabelAfter, 'slot-label-after', '<!-- slot="label-after" -->')}${slotted(slotFilter, 'slot-filter', '<!-- slot="filter" -->')}${slotted(slotDefault, 'slot-default')}</p-select>`,
