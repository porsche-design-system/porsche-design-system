...
export default {
  example: figma.code`<PSelect${hideLabel ? ' hideLabel={true}' : ''}${filter ? ' filter={true}' : ''}${label ? ` label="${label}"` : ''}${description ? ` description="${description}"` : ''}${message ? ` message="${message}"` : ''}${required ? ' required={true}' : ''}${value ? ` value="${value}"` : ''} state="${state}" dropdownDirection="${dropdownDirection}"${compact ? ' compact={true}' : ''}${disabled ? ' disabled={true}' : ''}>${slotted(slotFilter, 'slot-filter', '{/* slot="filter" */}')}${slotted(slotLabelAfter, 'slot-label-after', '{/* slot="label-after" */}')}${slotted(slotDefault, 'slot-default')}</PSelect>`,
  imports: [
    '// Docs: https://designsystem.porsche.com/v4/components/select/api',
    "import { PSelect } from '@porsche-design-system/components-react';",
  ],
  id: 'p-select',
