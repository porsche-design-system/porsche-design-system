...
export default {
  example: figma.code`<p-banner${dismissButton ? ' dismiss-button="true"' : ''}${heading ? ` heading="${heading}"` : ''}${headingTag ? ` heading-tag="${headingTag}"` : ''} state="${state}">${slotDefault}</p-banner>`,
