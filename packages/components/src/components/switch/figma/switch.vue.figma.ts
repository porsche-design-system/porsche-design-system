...
export default {
  example: figma.code`<PSwitch :alignLabel="'${alignLabel}'"${hideLabel ? ' :hideLabel="true"' : ''}${stretch ? ' :stretch="true"' : ''}${checked ? ' :checked="true"' : ''}${disabled ? ' :disabled="true"' : ''}${loading ? ' :loading="true"' : ''}${compact ? ' :compact="true"' : ''}>${slotDefault}</PSwitch>`,
  imports: [
