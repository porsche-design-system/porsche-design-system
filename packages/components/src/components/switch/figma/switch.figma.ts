...
export default {
  example: figma.code`<p-switch align-label="${alignLabel}"${hideLabel ? ' hide-label="true"' : ''}${stretch ? ' stretch="true"' : ''}${checked ? ' checked="true"' : ''}${disabled ? ' disabled="true"' : ''}${loading ? ' loading="true"' : ''}${compact ? ' compact="true"' : ''}>${slotDefault}</p-switch>`,
