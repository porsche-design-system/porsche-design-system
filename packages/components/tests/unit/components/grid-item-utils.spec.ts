...
describe('getDynamicCss()', () => {
  ...
  it.each(dataSizes.map((x) => JSON.stringify(x)))('should return correct css for size: %s', (size: GridItemSize) => {
  ...
  it.each(dataOffsets.map((x) => JSON.stringify(x)))(
    'should return correct css for offset: %s',
  ...
  it.each(
    dataSizes.map<[GridItemSize, GridItemOffset]>((x, i) => [JSON.stringify(x), dataOffsetsReversed[i]])
