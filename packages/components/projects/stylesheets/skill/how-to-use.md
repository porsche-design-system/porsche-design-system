...
Production stylesheets load Porsche Next from the bundled `stylesheets/fonts/` directory, without a font CDN request.
CSS bundlers resolve these relative URLs automatically. When copying the CSS manually, preserve the adjacent `fonts/`
directory and the `cn/` subdirectory layout.
