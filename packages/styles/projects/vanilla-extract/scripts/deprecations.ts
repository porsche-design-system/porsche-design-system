...
/** Domain keys in `vanillaExtractMeta` order — the order the generated list keeps. */
const DOMAINS = Object.keys(vanillaExtractMeta) as (keyof typeof vanillaExtractMeta)[];
...
const deprecatedDomains = (): (keyof typeof vanillaExtractMeta)[] => {
  ...
  return domains as (keyof typeof vanillaExtractMeta)[];
};
...
export const buildVanillaExtractDeprecations = (): Deprecations => {
