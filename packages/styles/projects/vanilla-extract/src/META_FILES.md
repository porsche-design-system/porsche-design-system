...
Each `*.meta.ts` file describes a group of style tokens. A build script (`scripts/generateVanillaExtractStyles.ts`, run
via `npm run generate:styles`) reads every meta file and writes one standalone `.ts` file per entry into a `generated/`
