 ...
 * {@link vanillaExtractMeta}. Produces a self-describing overview of the package — a short intro, a
...
/** Keep only the leaves of a given {@link VanillaExtractKind}, pruning emptied groups. Returns `undefined` when nothing remains. */
const pruneByKind = (node: VanillaExtractBranch, kind: VanillaExtractKind): VanillaExtractBranch | undefined => {
  if (isLeaf(node)) {
    return kindOf(node as VanillaExtractNode) === kind ? node : undefined;
  }
  const entries = (Array.isArray(node) ? node.map((n, i) => [i, n] as const) : Object.entries(node))
    .map(([key, value]) => [key, pruneByKind(value as VanillaExtractBranch, kind)] as const)
    .filter((entry): entry is [string | number, VanillaExtractBranch] => entry[1] !== undefined);
  if (entries.length === 0) {
    return undefined;
  }
...
/** Split the flat `vanillaExtractMeta` catalog into a single-kind view, dropping domains/groups that end up empty. */
const catalogByKind = (kind: VanillaExtractKind): Record<string, VanillaExtractBranch> =>
  (pruneByKind(vanillaExtractMeta as VanillaExtractBranch, kind) ?? {}) as Record<string, VanillaExtractBranch>;

const tokenOutline = deriveOutline<VanillaExtractToken>(catalogByKind('token'));

const utilityOutline = deriveOutline<VanillaExtractUtility>(catalogByKind('utility'));
...
/**
 * Render the full Vanilla Extract package overview as markdown. Pure function over
 * {@link vanillaExtractMeta}; the build script is responsible for writing it to disk.
 */
