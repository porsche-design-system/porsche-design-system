...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-input-email/examples/Default.ts](./p-input-email/examples/Default.ts) |
| Form | The `p-input-email` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-input-email/examples/Form.ts](./p-input-email/examples/Form.ts) |
| Slots | Use named slots when `label`, `description` or `message` need markup such as a link. | [./p-input-email/examples/Slots.ts](./p-input-email/examples/Slots.ts) |
