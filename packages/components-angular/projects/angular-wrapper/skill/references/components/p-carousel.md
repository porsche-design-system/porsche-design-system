...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-carousel/examples/Default.ts](./p-carousel/examples/Default.ts) |
| Slides with flexible widths | In case you want to have slides with different widths you can use `slidesPerPage` with a value of `auto`. | [./p-carousel/examples/FlexibleWidths.ts](./p-carousel/examples/FlexibleWidths.ts) |
| Jump to slide (activeSlideIndex) | To control the `p-carousel` from the outside you can specify its `activeSlideIndex` initially but also later. | [./p-carousel/examples/JumpToSlide.ts](./p-carousel/examples/JumpToSlide.ts) |
| Add/remove slides | Slides can be added and removed dynamically. | [./p-carousel/examples/DynamicSlides.ts](./p-carousel/examples/DynamicSlides.ts) |
| Centered Slide and Gradient Customization | The carousel centers the active slide and loops through slides individually rather than by page when multiple slides are visible. | [./p-carousel/examples/FocusOnCenterSlide.ts](./p-carousel/examples/FocusOnCenterSlide.ts) |
| Internationalization (i18n) | Default wordings for screen readers can be overridden or translated by passing an object to the `intl` property. | [./p-carousel/examples/Intl.ts](./p-carousel/examples/Intl.ts) |
