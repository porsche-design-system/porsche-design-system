...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-drilldown/examples/Default.ts](./p-drilldown/examples/Default.ts) |
| Example with custom content | To give more flexibility, it's possible to use custom slots `slot="button"` (renders a custom cascade button) and/or `slot="header"` (renders a custom header on mobile view). | [./p-drilldown/examples/CustomContent.ts](./p-drilldown/examples/CustomContent.ts) |
