...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-modal/examples/Default.ts](./p-modal/examples/Default.ts) |
| Scrollable modal with sticky footer | If the modal's content does not fit within the available space, the content becomes scrollable and the footer area remains sticky. | [./p-modal/examples/Scrollable.ts](./p-modal/examples/Scrollable.ts) |
| Modal as alert dialog | The `p-modal` component can be used as an alert dialog. | [./p-modal/examples/AlertDialog.ts](./p-modal/examples/AlertDialog.ts) |
| Custom styling | The modal component has some values that can be overridden via CSS custom properties (aka CSS variables). | [./p-modal/examples/CustomStyling.ts](./p-modal/examples/CustomStyling.ts) |
