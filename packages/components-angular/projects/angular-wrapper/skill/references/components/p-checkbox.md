...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-checkbox/examples/Default.ts](./p-checkbox/examples/Default.ts) |
| Form | The `p-checkbox` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-checkbox/examples/Form.ts](./p-checkbox/examples/Form.ts) |
| Indeterminate | Mask the visual appearance of a checkbox which has a state in-between checked and unchecked. | [./p-checkbox/examples/Indeterminate.ts](./p-checkbox/examples/Indeterminate.ts) |
| Slots | Use named slots when `label` or `message` need markup such as a link. | [./p-checkbox/examples/Slots.ts](./p-checkbox/examples/Slots.ts) |
| Custom wrapped label (🧪Experimental) | The `p-checkbox` component also supports a custom wrapped `label` by wrapping the component in a `label` tag. | [./p-checkbox/examples/WrappedLabel.ts](./p-checkbox/examples/WrappedLabel.ts) |
