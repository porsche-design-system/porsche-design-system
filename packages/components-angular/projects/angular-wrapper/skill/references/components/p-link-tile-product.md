...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-link-tile-product/examples/Default.ts](./p-link-tile-product/examples/Default.ts) |
| Framework routing (anchor nesting) | To support framework routing you can provide the link as a slotted element by using the `anchor` slot instead of using the `href` property. | [./p-link-tile-product/examples/FrameworkRouting.ts](./p-link-tile-product/examples/FrameworkRouting.ts) |
