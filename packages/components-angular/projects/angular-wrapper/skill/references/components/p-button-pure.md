...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-button-pure/examples/Default.ts](./p-button-pure/examples/Default.ts) |
| Button Pure with custom clickable/focusable area | Sometimes it might be useful to enlarge the clickable/focusable area of a button to fulfill accessibility guidelines. | [./p-button-pure/examples/CustomClickableArea.ts](./p-button-pure/examples/CustomClickableArea.ts) |
| Form | When used as a submit button, the `name` and `value` props are submitted as a pair as part of the form data. | [./p-button-pure/examples/Form.ts](./p-button-pure/examples/Form.ts) |
| Form Attribute | When a button is used as a submit or reset button outside a form, the `form` attribute can be utilized to explicitly associate the button with a specific form element. | [./p-button-pure/examples/FormAttribute.ts](./p-button-pure/examples/FormAttribute.ts) |
