...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-tag/examples/Default.ts](./p-tag/examples/Default.ts) |
| With slotted button | It is possible to add a `<button>` tag into the `p-tag` component. | [./p-tag/examples/SlottedButton.ts](./p-tag/examples/SlottedButton.ts) |
| With slotted link | It is possible to add an `<a>` tag to the `p-tag` component. | [./p-tag/examples/SlottedLink.ts](./p-tag/examples/SlottedLink.ts) |
| Multiline | The contents of the `p-tag` component are rendered with `white-space: nowrap` by default. | [./p-tag/examples/Multiline.ts](./p-tag/examples/Multiline.ts) |
