...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-button-tile/examples/Default.ts](./p-button-tile/examples/Default.ts) |
| UI behaviour | The component is able to break out of its aspect ratio in case content overflows to be accessibility compliant (see first row in example). | [./p-button-tile/examples/UiBehaviour.ts](./p-button-tile/examples/UiBehaviour.ts) |
| Hyphens | It is possible to overwrite the hyphens style on the host element and use 'soft' hyphens. | [./p-button-tile/examples/Hyphens.ts](./p-button-tile/examples/Hyphens.ts) |
| Footer slot | You can also use a `footer` slot to display additional text below the description, providing more flexibility for custom content. | [./p-button-tile/examples/FooterSlot.ts](./p-button-tile/examples/FooterSlot.ts) |
