...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-pin-code/examples/Default.ts](./p-pin-code/examples/Default.ts) |
| Form | The `p-pin-code` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-pin-code/examples/Form.ts](./p-pin-code/examples/Form.ts) |
| Copy+Paste and autocomplete | By default, only one input can be changed at a time. | [./p-pin-code/examples/CopyPaste.ts](./p-pin-code/examples/CopyPaste.ts) |
| Slots | Use named slots when `label`, `description` or `message` need markup such as a link. | [./p-pin-code/examples/Slots.ts](./p-pin-code/examples/Slots.ts) |
