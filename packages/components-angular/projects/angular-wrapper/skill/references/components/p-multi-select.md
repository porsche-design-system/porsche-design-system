...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-multi-select/examples/Default.ts](./p-multi-select/examples/Default.ts) |
| Form | The `p-multi-select` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-multi-select/examples/Form.ts](./p-multi-select/examples/Form.ts) |
| Slots | Use named slots when `label`, `description` or `message` need markup such as a link. | [./p-multi-select/examples/Slots.ts](./p-multi-select/examples/Slots.ts) |
| Set Value | The `p-multi-select` component behaves like regular form elements. | [./p-multi-select/examples/SetValue.ts](./p-multi-select/examples/SetValue.ts) |
| With optgroups | With optgroups | [./p-multi-select/examples/Optgroups.ts](./p-multi-select/examples/Optgroups.ts) |
| Custom asynchronous filtering | The `p-multi-select` component automatically filters options based on user input. | [./p-multi-select/examples/AsyncFilter.ts](./p-multi-select/examples/AsyncFilter.ts) |
| Custom option rendering | By default, `p-multi-select-option` only allows `#text` nodes. | [./p-multi-select/examples/SelectedSlot.ts](./p-multi-select/examples/SelectedSlot.ts) |
