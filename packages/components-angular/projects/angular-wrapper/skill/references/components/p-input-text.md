...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-input-text/examples/Default.ts](./p-input-text/examples/Default.ts) |
| Form | The `p-input-text` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-input-text/examples/Form.ts](./p-input-text/examples/Form.ts) |
| Controlled Input (Preventing User Input) | In some cases you may want to fully control the input value (for example to limit or validate user input). | [./p-input-text/examples/Controlled.ts](./p-input-text/examples/Controlled.ts) |
| Slots | Use named slots when `label`, `description` or `message` need markup such as a link. | [./p-input-text/examples/Slots.ts](./p-input-text/examples/Slots.ts) |
