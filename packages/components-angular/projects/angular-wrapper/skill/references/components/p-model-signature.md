...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-model-signature/examples/Default.ts](./p-model-signature/examples/Default.ts) |
| Safe Zone | The `safe-zone` prop's default is `true`, which ensures a visual balance across all model signatures. | [./p-model-signature/examples/SafeZone.ts](./p-model-signature/examples/SafeZone.ts) |
| Mask: Blend Mode | If `p-model-signature` should be blended with its background, the CSS property [mix-blend-mode](https://developer.mozilla.org/en-US/docs/Web/CSS/mix-blend-mode) can be used (`{{ blendMode }}` will produce the best results). | [./p-model-signature/examples/MaskBlendMode.ts](./p-model-signature/examples/MaskBlendMode.ts) |
| Mask: Image | Mask: Image | [./p-model-signature/examples/MaskImage.ts](./p-model-signature/examples/MaskImage.ts) |
| Mask: Video | Mask: Video | [./p-model-signature/examples/MaskVideo.ts](./p-model-signature/examples/MaskVideo.ts) |
| Custom styling | The `p-model-signature` color can be changed by setting the `color` prop to `inherit` and applying some CSS `color`. | [./p-model-signature/examples/CustomStyling.ts](./p-model-signature/examples/CustomStyling.ts) |
