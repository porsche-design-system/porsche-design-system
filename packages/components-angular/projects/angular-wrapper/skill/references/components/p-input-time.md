...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-input-time/examples/Default.ts](./p-input-time/examples/Default.ts) |
| Form | The `p-input-time` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-input-time/examples/Form.ts](./p-input-time/examples/Form.ts) |
| Slots | Use named slots when `label`, `description` or `message` need markup such as a link. | [./p-input-time/examples/Slots.ts](./p-input-time/examples/Slots.ts) |
