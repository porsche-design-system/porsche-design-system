...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-divider/examples/Default.ts](./p-divider/examples/Default.ts) |
| Vertical | **Hint:** The component has not an implicit vertical height unless we set a container which provides it or define the height on the component itself. | [./p-divider/examples/Vertical.ts](./p-divider/examples/Vertical.ts) |
| Responsive | You can change the divider's direction by using different major breakpoints `xs`, `s`, `m`, `l`, `xl`. | [./p-divider/examples/Responsive.ts](./p-divider/examples/Responsive.ts) |
