...
| --- | --- | --- |
| Default | Minimal default configuration. | [./p-input-date/examples/Default.ts](./p-input-date/examples/Default.ts) |
| Form | The `p-input-date` can be integrated into a form in two ways: **controlled** or **uncontrolled**, depending on your needs. | [./p-input-date/examples/Form.ts](./p-input-date/examples/Form.ts) |
| Slots | Use named slots when `label`, `description` or `message` need markup such as a link. | [./p-input-date/examples/Slots.ts](./p-input-date/examples/Slots.ts) |
