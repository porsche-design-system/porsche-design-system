...
const getInput = (page: Page) => page.locator('p-input-week input');

const setValue = async (page) => {
  const host = getHost(page);
  const input = getInput(page);

  await input.fill('2025-W26');
...
test.describe('Angular forms integration', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/input-week-example-reactive-form');
    const host = getHost(page);
  ...
  test('setting value manually updates form state correctly', async ({ page }) => {
    ...
    await expect(page.locator('[data-field="disabled"]')).toHaveText('false');
    await expect(page.locator('[data-field="value"]')).toHaveText('2025-W26');
  ...
  test('submitted form contains correct information', async ({ page }) => {
    ...
    await expect(page.locator('[data-field="submitted"]')).toHaveText('{"myInputWeek":"2025-W26"}');
  });
});
