...
### Viewport range 
In order to provide consistency throughout all Porsche web
experiences, all layouts should be optimized for a viewport range from **320 px** to a maximum of **1920 px**. For viewport sizes from 1920 px onwards, a
...
All content elements that should behave
...
should be defined by using the [Spacing](#/layout/spacing) component.
...
stacked on mobile, using 12 grid columns. 
- … text sizes – commonly using smaller text on mobile, larger text on bigger screens. 
- … the general layout of the component by using different patterns on mobile and desktop. 
Example: Turning a multi-column teaser module on desktop into an accordion on mobile.
...
It is possible to place elements
