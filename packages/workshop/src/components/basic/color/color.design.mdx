...
Depending on which Porsche Theme you're using, you should stick to a limited color set deriving from the Porsche Color Palette, as these colors
are optimized to match accessibility (Level AA) criteria within the specific Theme
...
Within certain limits, it's allowed to mix the Porsche Color Themes – let's say, for example, you could integrate a larger 
Dark Theme area in your Light Theme to visually distinguish the footer from the content. 
...
taking different factors into account, such as target group, use flows (visual
entry and exit of your application) or business goals, for example. When in doubt:
For web applications, the Porsche Light Theme should be the first choice.
Once you've selected a Color Theme, stick with it throughout your whole application to ensure consistency. 

### Step 2: Setup
Set up your layout, using the **page background color** of your Porsche Color Theme of choice.
To quickstart, you can use the [Sketch Layout Template](http://ui.porsche.com/latest/porsche-ui-kit-layout-template.sketch) that provides both light and dark themed artboards.

### Step 3: 
To define the colors to be used within in the screen, you should check the **information level** of the respective elements.
Some general rules: 

- Use darker/lighter grey shades to make a surface stand out from the page background or the surface underneath. The ligher/darker the color is, the more attention it gets.
- For larger screen areas make sure only the dedicated "Surface" color for your Theme (e.g. "Surface Light"). Just let the proportional scale of the Theme guide you.
- For smaller areas, such as tiles, or other graphical elements you can use further grey shades. Stick either to the accessibility-safe "Additional Colors" of your Theme
(highly recommended for informative content) or, for decorative purpose, to any other grey shade from the Porsche Neutral Palette.
- Use Porsche Red sparingly and only for highlighting single elements.
