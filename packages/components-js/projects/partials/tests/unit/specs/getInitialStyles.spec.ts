...
describe('format: sha256', () => {
  it('should return hash for core styles', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-2cCbtCpeatLQ2B1atXNmpZSo3g6MAoNsfXQ+/uKb8Vo='"`);
  });
  ...
  it('should return hash for core styles with custom prefix', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-rSphxrbdPtX1Yyk/mZVdcEUfeVo1MR+xXJ3C3okC4WE='"`);
  });
  ...
  it('should return hash for core styles with multiple custom prefixes', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-HI1x7NJJQvxn8q6YAblJI/vjhFNnJe0ssznedRvXmOw='"`);
  });
});
