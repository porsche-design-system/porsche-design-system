...
describe('format: sha256', () => {
  it('should return hash for core styles', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-cR60Vd3dzEk1qg6kWot0st8a6Z5cAenqjMb1oYtjtQM='"`);
  });
  ...
  it('should return hash for core styles with custom prefix', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-ULVC2ZQM3nWSG483pVrv1q2aHBTdIaDikBBfgUW2gNI='"`);
  });
  ...
  it('should return hash for core styles with multiple custom prefixes', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-52c5OITJLFGI4QWqaqTBTV4ujwvtnisR7PdWkjWG0Oc='"`);
  });
});
