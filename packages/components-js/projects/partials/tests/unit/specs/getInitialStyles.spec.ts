...
describe('format: sha256', () => {
  it('should return hash for core styles', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-gdUGWsgLuRzoo06nwK+NmDGq0HxxfTA7/NMzgTKcP2w='"`);
  });
  ...
  it('should return hash for core styles with custom prefix', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-EFPmvSn78mCBzS+dYYPnnAFcC6rTlpqIero831oQznE='"`);
  });
  ...
  it('should return hash for core styles with multiple custom prefixes', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-Ha/gW1pI9w+TY2pKb9KesGCeisn9eUzSNj5Gyu+wNr4='"`);
  });
});
