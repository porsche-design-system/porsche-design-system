...
describe('format: sha256', () => {
  it('should return hash for core styles', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-s/9SKqj/gvxc3Njd+usmPRJP1iwcH1jVjIO++DFTlfI='"`);
  });
  ...
  it('should return hash for core styles with custom prefix', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-kyAU4O5jyRl2pFVp+P3ZTGRXEZt0iwQYc/CKSQfTqlI='"`);
  });
  ...
  it('should return hash for core styles with multiple custom prefixes', async () => {
    ...
    expect(result).toMatchInlineSnapshot(`"'sha256-gLkaHy2/vs3wmzVCbZSMepP+VD4JTAh913LBr/WilO0='"`);
  });
});
