...
describe('format: html', () => {
  it('should return core styles', () => {
    ...
    const regex = new RegExp(
      `<style data-pds-initial-styles>${tagNames}{visibility:hidden}.hydrated{visibility:inherit}</style>`
    );
  ...
  it('should add custom prefixes to component names', () => {
    ...
    const regex = new RegExp(
      `<style data-pds-initial-styles-custom-prefix>${prefixedTagNames}{visibility:hidden}.hydrated{visibility:inherit}</style>`
    );
    expect(result).toMatch(regex);
  });
});
