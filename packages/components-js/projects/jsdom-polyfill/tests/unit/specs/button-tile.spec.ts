...
it('should expose its nested button to shadow queries', async () => {
  ...
  const { shadowRoot } = document.querySelector('p-button-tile');
  expect(screen.getByShadowRole('button')).toBe(
    shadowRoot.querySelector('p-button').shadowRoot.querySelector('button.root')
  );
});
