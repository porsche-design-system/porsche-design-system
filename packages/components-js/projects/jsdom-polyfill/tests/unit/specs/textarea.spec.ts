...
it('should have initialized shadow dom', async () => {
  document.body.innerHTML = getMarkup('p-textarea');
...
it('should expose its label to shadow queries', async () => {
  ...
  const shadowRoot = document.querySelector('p-textarea')?.shadowRoot;
  assertDefined(shadowRoot);
  expect(screen.getByShadowLabelText('Some label')).toBe(shadowRoot.querySelector('textarea'));
});
