...
it('should have working events', async () => {
  ...
  el.addEventListener('update', (e) => {
    ...
    span.innerHTML = (e as CustomEvent).detail.activeTabIndex;
...
it('should expose its tablist to shadow queries', async () => {
  ...
  const shadowRoot = document.querySelector('p-tabs-bar')?.shadowRoot;
  assertDefined(shadowRoot);
  const scrollerShadowRoot = shadowRoot.querySelector('p-scroller')?.shadowRoot;
  assertDefined(scrollerShadowRoot);
  expect(screen.getByShadowRole('tablist')).toBe(scrollerShadowRoot.querySelector('div.scroll'));
});
