...
it('should expose its tab buttons to shadow queries', async () => {
  ...
  const shadowRoot = document.querySelector('p-tabs')?.shadowRoot;
  assertDefined(shadowRoot);
  const tabsBar = shadowRoot.querySelector('p-tabs-bar');
  assertDefined(tabsBar);
  expect(screen.getAllByShadowRole('button', { name: 'Some label' })).toEqual([...tabsBar.querySelectorAll('button')]);
});
