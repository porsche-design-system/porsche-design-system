...
const getOverviewPageMarkup = (): string => {
...
const getJsdomPolyfillBuild = (): string => {
  const filePath = path.resolve(componentsJsEntry, '../jsdom-polyfill/index.js');
...
it('should have no cdn requests', async () => {
  ...
  const overviewPageMarkup = getOverviewPageMarkup();
  ...
  expect(await button.evaluate((el) => el.shadowRoot !== null), 'shadowRoot is defined').toBeTruthy();
