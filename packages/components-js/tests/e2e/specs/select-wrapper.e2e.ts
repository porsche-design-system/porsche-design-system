...
describe('select-wrapper', () => {
  ...
  describe('fake drop down', () => {
    ...
    `
      ...
      const activeDescendant = async () => await getAttribute(await getSelectOptionList(), 'aria-activedescendant');
      ...
      expect(await getElementPosition(await getSelectOptionList(), '.p-select-wrapper__fake-option--highlighted')).toBe(
        0
      );
      ...
      // 1 x arrow down should jump over disabled fake option
      await select.press('ArrowDown');
      await waitForStencilLifecycle(page);
      ...
      // 1 x arrow up should jump over disabled fake option
  ...
  describe('fake drop down with filter (combobox)', () => {
    it('should render', async () => {
      ...
      const fakeOptionList = await getSelectOptionList();
      const fakeOptionFilter = await selectNode(
        page,
        'p-select-wrapper >>> .p-select-wrapper__filter-input'
      );
      const fakeOptionSelected = await selectNode(
        page,
        'p-select-wrapper >>> .p-select-wrapper__fake-option--selected'
