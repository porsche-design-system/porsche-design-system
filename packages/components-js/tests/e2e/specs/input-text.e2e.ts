...
test.describe('value', () => {
  ...
  test('should allow controlled input via programmatic value updates in input listener', async ({ page }) => {
    await initInputText(page, { props: { name: 'some-name' } });
    const host = getHost(page);
    const inputText = getInputText(page);
    ...
    await page.evaluate(() => {
      const hostElement = document.querySelector('p-input-text');
    ...
    await expect(inputText).toHaveValue('b');
...
test.describe('form', () => {
  ...
  test('should omit value from FormData when input is disabled (matching native behavior)', async ({ page }) => {
    ...
    const value = '10';
    await initInputText(page, {
      props: { name, value, disabled: true },
      isWithinForm: true,
      markupAfter: '<button type="submit">Submit</button>',
    });
    const form = getForm(page);
  ...
  test('should include value in FormData when input is readOnly (matching native behavior)', async ({ page }) => {
    ...
    const value = '10';
    await initInputText(page, {
      props: { name, value, readOnly: true },
      isWithinForm: true,
      markupAfter: '<button type="submit">Submit</button>',
    });
    const form = getForm(page);
