...
const scenario = async (page: Page, theme: Theme, scheme?: PrefersColorScheme): Promise<void> => {
  ...
  // Hover states
  await forceHoverState(page, '.hover p-input-time:not(.force-label) >>> .wrapper');
  await forceHoverState(page, '.hover p-input-time >>> p-button-pure >>> button');

  // Focus states
  await forceFocusState(page, '.focus p-input-time:not(.force-label) >>> input');
  await forceFocusState(page, '.focus p-input-time.force-label >>> label');
  ...
  await forceFocusHoverState(page, '.focus-hover p-input-time.force-label >>> label');
