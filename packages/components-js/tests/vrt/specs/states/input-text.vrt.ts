...
const scenario = async (page: Page, theme: Theme, scheme?: PrefersColorScheme): Promise<void> => {
  ...
  const markup = () => `
    ...
    <div>
      <p-input-text class="force-label" label="Label gets hovered or focussed" value="Hello"></p-input-text>
    </div>`;
  ...
  // Hover states
  await forceHoverState(page, '.hover p-input-text:not(.force-label) input');
  await forceHoverState(page, '.hover p-input-text.force-label >>> label');
  await forceHoverState(page, '.hover p-input-text a');
  await forceHoverState(page, '.hover p-input-text >>> p-button-pure >>> button');

  // Focus states
  await forceFocusState(page, '.focus p-input-text input');
  await forceFocusVisibleState(page, '.focus p-input-text a');

  // Focus + hover states
  await forceFocusHoverState(page, '.focus-hover p-input-text:not(.force-label) input');
  await forceFocusState(page, '.focus-hover p-input-text.force-label input');
  await forceHoverState(page, '.focus-hover p-input-text.force-label >>> label');
  await forceFocusHoverState(page, '.focus-hover p-input-text a');
};
