...
const amountOfTestableComponents = 56;
...
for (const component of components.filter(
  (component) =>
    !['button-tile', 'link-tile', 'link-tile-model-signature', 'input-text', 'input-email'].includes(component) // TODO: fix "Ensure the contrast between foreground and background colors meets WCAG 2 AA minimum contrast ratio thresholds" for disabled `input-text` with counter
)) {
