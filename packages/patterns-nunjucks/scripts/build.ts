...
import { createEnvironment, renderTemplate } from '../plugins/nunjucks.ts';
...
/**
 * Renders every `.html` file with Nunjucks and copies all other files verbatim.
...
const build = (): void => {
  ...
  // One environment for the whole build – the loader cache is what makes repeated partial lookups cheap.
  const environment = createEnvironment(srcDir);
  ...
  for (const relativePath of files) {
    ...
    if (path.extname(relativePath) === '.html') {
      const html = renderTemplate(fs.readFileSync(sourcePath, 'utf8'), {
        rootDir: srcDir,
        filePath: sourcePath,
        environment,
      });
